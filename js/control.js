const controlViews = {
    ...
    filterPcStatus() {
        ...
        const filtered = app.hosts.filter(h => {
    ...
    filterLogs() {
        const keyword = document.getElementById('search-input').value.toLowerCase();
        const filtered = app.logs.filter(l => l.host.toLowerCase().includes(keyword) || l.msg.toLowerCase().includes(keyword) || l.type.toLowerCase().includes(keyword));
    ...
    filterGroupManage() {
        ...
        const filtered = app.hosts.filter(h => h.name.toLowerCase().includes(keyword) || h.id.toLowerCase().includes(keyword));
    ...
    openReportPreview(id) {
        ...
        let filteredHosts = app.hosts;
        if(report.group === 'vip') filteredHosts = app.hosts.filter(h => h.isVip);
