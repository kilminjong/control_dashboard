const dataViews = {
    ...
    getB2BData() { return DB.getB2B(); },
    getTaxData() { return DB.getTax(); },
    getAssetData() { return DB.getAssets(); },
    getCardData() { return DB.getCards(); },
    ...
    buildB2BRows(data) {
        ...
        return data.map(item => `
            <tr class="hover:bg-slate-50 transition-colors">
                ...
                <td class="px-4 py-3">
                    ${item.status === 'target' 
                        // 💡 [수정] 대출 제안 버튼을 클릭하면 제안 발송 모달창 오픈!
                        ? `<button onclick="app.openProposalModal('${item.name}', 'B2B/전자어음 기반 대출(여신) 제안')" class="text-[11px] font-bold px-3 py-1.5 bg-blue-50 text-blue-700 border border-blue-200 rounded hover:bg-blue-100 transition shadow-sm">대출(여신) 제안</button>`
    ...
    renderB2B() {
        ...
        <div class="max-w-[1400px] mx-auto flex flex-col h-full">
            ...
            <div class="flex justify-between items-end mb-3">
                ...
                <div class="flex space-x-2"><button class="bg-slate-800 text-white px-3 py-1.5 rounded text-xs font-bold hover:bg-slate-700 shadow-sm"><i class="fa-solid fa-paper-plane mr-1.5"></i>선택 항목 영업점 일괄 전송</button></div>
            </div>
                ...
                <div class="overflow-y-auto max-h-[550px] relative">
                    <table class="w-full text-left border-collapse whitespace-nowrap">
                        <thead class="bg-white text-[12px] text-slate-500 border-b border-slate-200 sticky top-0 z-10 shadow-sm">
                            <tr><th class="px-4 py-3 font-bold w-10 text-center"><input type="checkbox" class="rounded border-slate-300"></th><th class="px-4 py-3 font-bold">고객사명 (사업자번호)</th><th class="px-4 py-3 font-bold">금융기관 / 수집 상품</th><th class="px-4 py-3 font-bold text-right">만기 예정 금액 (원)</th><th class="px-4 py-3 font-bold text-center">만기 일자 (D-Day)</th><th class="px-4 py-3 font-bold">마케팅 조치 (Action)</th></tr>
                        </thead>
    ...
    renderAsset() { 
        return `<div class="max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-4"><div class="bg-white border border-slate-200 rounded p-5 shadow-sm h-fit"><h3 class="font-bold text-slate-800 text-[14px] mb-1">전체 고객사 금융기관별 자산 점유율 현황</h3><p class="text-[11px] text-slate-500 mb-5">수집된 금융 데이터 합산 기준 (단위: 억 원)</p><div class="h-[300px] w-full"><canvas id="assetChart"></canvas></div></div><div class="bg-white border border-slate-200 rounded p-5 flex flex-col shadow-sm max-h-[700px]"><div class="flex justify-between items-center mb-4"><h3 class="font-bold text-slate-800 text-[14px]">고액 예·적금 만기 도래 타겟 리스트</h3><input type="text" id="asset-search" onkeyup="dataViews.filterAsset()" placeholder="고객사, 은행명 검색" class="border border-slate-300 rounded text-xs px-3 py-1 w-48 outline-none focus:border-teal-500 font-medium"></div><div id="asset-list" class="flex-1 overflow-y-auto pr-2 space-y-1">${this.buildAssetRows(this.getAssetData())}</div></div></div>`; 
    },
    initAssetChart() { 
        const ctx = document.getElementById('assetChart').getContext('2d'); 
        const chart = new Chart(ctx, { type: 'bar', data: { labels: ['당행(하나)', 'A은행(국민)', 'B은행(신한)', 'C은행(기업)'], datasets: [{ label: '보유 자산 총액', data: [850, 450, 320, 150], backgroundColor: ['#0d9488', '#cbd5e1', '#cbd5e1', '#cbd5e1'], borderRadius: 4, barThickness: 40 }] }, options: { responsive: true, maintainAspectRatio: false, scales: { y: { beginAtZero: true, grid: { borderDash: [2, 2], color: '#f1f5f9' }, border: { display: false } }, x: { grid: { display: false }, border: { display: false } } }, plugins: { legend: { display: false } } } }); 
    ...
    buildCardRows(data) {
        ...
        return data.map(item => `
            <tr class="hover:bg-slate-50 transition-colors">
                ...
                <td class="px-4 py-3 text-right font-mono font-bold text-slate-800 text-[13px]">${item.amount.toLocaleString()}</td>
                <td class="px-4 py-3 text-center"><button onclick="app.openProposalModal('${item.name}', '주유 특화 법인카드 제안')" class="text-[11px] font-bold px-3 py-1.5 bg-blue-50 text-blue-700 border border-blue-200 rounded hover:bg-blue-100 transition shadow-sm">맞춤 카드 제안</button></td>
            </tr>
        `).join('');
    },
    filterCard() {
    ...
    renderCard() { 
        return `<div class="max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-6"><div class="lg:col-span-4 bg-white border border-slate-200 rounded p-6 shadow-sm h-fit"><h3 class="font-bold text-slate-800 text-[14px] mb-1">타행 법인카드 주 사용처 분석 (업종별)</h3><p class="text-[11px] text-slate-500 mb-6">당행 신규 카드 상품 기획 및 전환 영업 타겟팅 데이터</p><div class="h-[220px] flex justify-center"><canvas id="cardChart"></canvas></div><div class="mt-6 pt-4 border-t border-slate-100 bg-slate-50 p-4 rounded text-center"><p class="text-[12px] text-slate-700 font-medium leading-relaxed">수집된 데이터 분석 결과, 타행 카드 결제액의 40% 이상이 <span class="font-bold text-teal-600">주유/교통</span>에 집중되어 있습니다. <br>따라서 <span class="font-bold text-slate-900 underline">당행 주유 특화 법인카드 프로모션</span>을 우선적으로 제안해야 합니다.</p></div></div><div class="lg:col-span-8 bg-white border border-slate-200 rounded flex flex-col shadow-sm overflow-hidden"><div class="px-4 py-3 border-b border-slate-200 bg-slate-50 flex justify-between items-center"><h3 class="text-[14px] font-bold text-slate-800">법인카드 고액 결제 기업 타겟 리스트</h3><div class="relative"><i class="fa-solid fa-magnifying-glass absolute left-3 top-2 text-slate-400 text-xs"></i><input type="text" id="card-search" onkeyup="dataViews.filterCard()" placeholder="고객사, 사용처, 카드사 검색" class="border border-slate-300 rounded text-xs pl-8 pr-3 py-1.5 w-64 outline-none focus:border-teal-500 font-medium"></div></div><div class="overflow-y-auto max-h-[600px] relative"><table class="w-full text-left border-collapse whitespace-nowrap"><thead class="bg-white text-[12px] text-slate-500 border-b border-slate-200 sticky top-0 z-10 shadow-sm"><tr><th class="px-4 py-3 font-bold">고객사명</th><th class="px-4 py-3 font-bold">주 사용 타행카드</th><th class="px-4 py-3 font-bold">주 결제 업종</th><th class="px-4 py-3 font-bold text-right">월 평균 결제액 (원)</th><th class="px-4 py-3 font-bold text-center">영업(Action)</th></tr></thead><tbody id="card-tbody" class="text-xs text-slate-700 divide-y divide-slate-100">${this.buildCardRows(this.getCardData())}</tbody></table></div></div></div>`; 
    },
    initCardChart() { 
        const ctx = document.getElementById('cardChart').getContext('2d'); 
        const chart = new Chart(ctx, { type: 'doughnut', data: { labels: ['주유/교통비', '항공/숙박(출장)', '식대/접대비', '공과금/기타'], datasets: [{ data: [40, 35, 15, 10], backgroundColor: ['#0d9488', '#3b82f6', '#94a3b8', '#cbd5e1'], borderWidth: 2, borderColor: '#fff' }] }, options: { responsive: true, maintainAspectRatio: false, cutout: '65%', plugins: { legend: { position: 'right', labels: { boxWidth: 10, font: { size: 11, family: 'Pretendard' } } } } } }); 
        app.chartInstances.push(chart); 
    },
        ...
        return data.map(item => `
            <tr class="hover:bg-slate-50 transition-colors">
                ...
                <td class="px-4 py-3 text-center">
                    ${item.status === '우량' 
                        // 💡 [수정] 뱃지를 클릭하면 여신 상향 제안 모달창 오픈!
                        ? `<button onclick="app.openProposalModal('${item.partner}', '매출 우량에 따른 기업 여신 한도 상향 제안')" class="text-[11px] text-teal-700 font-bold border border-teal-200 bg-teal-50 px-2.5 py-1 rounded-full shadow-sm hover:bg-teal-100 transition">매출 우량 (여신 상향 제안)</button>`
    ...
    renderTax() { 
        return `<div class="max-w-[1400px] mx-auto flex flex-col h-full"><div class="grid grid-cols-2 gap-4 mb-4"><div class="bg-white border border-slate-200 p-4 rounded flex justify-between items-center shadow-sm"><div class="text-slate-800"><p class="text-[12px] font-bold text-slate-500 mb-1">당월 총 매출 세금계산서 수집액</p><h3 class="text-2xl font-black font-mono text-blue-700">3,450<span class="text-sm font-bold ml-1 text-slate-600">억 원</span></h3></div><i class="fa-solid fa-arrow-trend-up text-3xl text-blue-100"></i></div><div class="bg-white border border-slate-200 p-4 rounded flex justify-between items-center shadow-sm"><div class="text-slate-800"><p class="text-[12px] font-bold text-slate-500 mb-1">당월 총 매입 세금계산서 수집액</p><h3 class="text-2xl font-black font-mono text-red-600">1,210<span class="text-sm font-bold ml-1 text-slate-600">억 원</span></h3></div><i class="fa-solid fa-arrow-trend-down text-3xl text-red-100"></i></div></div><div class="flex justify-between items-end mb-3 mt-2"><h2 class="text-[15px] font-bold text-slate-800 tracking-tight">수집된 세금계산서 기반 매입/매출 장부 (기업 여신 평가용)</h2><button class="border border-slate-300 bg-white px-3 py-1.5 rounded text-xs font-bold text-slate-600 hover:bg-slate-50 shadow-sm"><i class="fa-solid fa-download mr-1.5"></i>전체 원장 엑셀 다운로드</button></div><div class="bg-white border border-slate-200 rounded overflow-hidden shadow-sm flex flex-col flex-1"><div class="px-4 py-3 border-b border-slate-200 bg-slate-50 flex space-x-3 items-center"><div class="relative"><i class="fa-solid fa-magnifying-glass absolute left-3 top-2 text-slate-400 text-xs"></i><input type="text" id="tax-search" onkeyup="dataViews.filterTax()" placeholder="거래처명, 품목, 구분(매출/매입) 검색" class="border border-slate-300 rounded text-xs pl-8 pr-3 py-1.5 w-72 outline-none focus:border-teal-500 font-medium"></div></div><div class="overflow-y-auto max-h-[550px] relative"><table class="w-full text-left whitespace-nowrap"><thead class="bg-white text-[12px] text-slate-500 border-b border-slate-200 sticky top-0 z-10 shadow-sm"><tr><th class="px-4 py-3 font-bold w-24 text-center">발행 구분</th><th class="px-4 py-3 font-bold">거래처 (발행처)</th><th class="px-4 py-3 font-bold">품목 설명</th><th class="px-4 py-3 font-bold text-right">공급가액 합계 (원)</th><th class="px-4 py-3 font-bold text-center">여신/한도 평가 시스템 상태</th></tr></thead><tbody id="tax-tbody" class="text-xs text-slate-700 divide-y divide-slate-100">${this.buildTaxRows(this.getTaxData())}</tbody></table></div></div></div>`; 
    }
};