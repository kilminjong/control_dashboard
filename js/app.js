const app = {
    ...
    chartInstances: [],
    c360Charts: [], // 신규: 360 패널 내부 차트 관리용
    ...
    init() {
        this.generateData(); 
        this.injectCustomer360Panel(); // 신규: 360 패널 HTML 주입
        const firstMenu = document.querySelector('[data-menu]');
        this.loadView('dashboard', firstMenu);
    },

    // 💡 [신규 추가] 750px 대형 Customer 360 심층 분석 패널 HTML 주입
    ...
    openCustomer360(companyName) {
        ...
        const radar = new Chart(ctxRadar, {
            type: 'radar',
            data: { labels: ['타행자산 보유', '여신(대출) 필요도', '카드 결제액', '어음/B2B 거래량', '당행 기여도'], datasets: [{ data: [Math.floor(Math.random()*60)+40, Math.floor(Math.random()*60)+40, Math.floor(Math.random()*80)+20, Math.floor(Math.random()*80)+20, Math.floor(Math.random()*50)+10], backgroundColor: 'rgba(20, 184, 166, 0.2)', borderColor: 'rgba(20, 184, 166, 1)', pointBackgroundColor: 'rgba(20, 184, 166, 1)', borderWidth: 2 }] },
            options: { responsive: true, maintainAspectRatio: false, scales: { r: { angleLines: { color: '#e2e8f0' }, grid: { color: '#e2e8f0' }, pointLabels: { font: { size: 10, family: 'Pretendard', weight: 'bold' }, color: '#64748b' }, ticks: { display: false, max: 100 } } }, plugins: { legend: { display: false } } }
        });
        ...
        const bar = new Chart(ctxBar, {
            type: 'bar',
            data: {
                labels: ['10월', '11월', '12월', '1월', '2월', '당월'],
                datasets: [
                    { label: '매출액', data: [120, 150, 180, 130, 200, 250], backgroundColor: '#0ea5e9', borderRadius: 2 },
                    { label: '지출액', data: [90, 110, 100, 120, 150, 140], backgroundColor: '#f43f5e', borderRadius: 2 }
                ]
            },
            options: { responsive: true, maintainAspectRatio: false, scales: { y: { beginAtZero: true, grid: { borderDash: [2, 2], color: '#f1f5f9' }, border: { display: false }, ticks: { font: {size: 9} } }, x: { grid: { display: false }, border: { display: false }, ticks: { font: {size: 10} } } }, plugins: { legend: { position: 'bottom', labels: { boxWidth: 8, font: {size: 10} } } } }
        });
    ...
    generateData() {
        ...
        for(let i=1; i<=300; i++) {
            ...
            this.hosts.push({
                ...
                isVip: i % 8 === 0,
                terminals: Math.floor(Math.random() * 200) + 10,
    ...
    loadViewWithFilter(viewName, filterType) {
        ...
        setTimeout(() => {
            ...
            if (viewName === 'pcStatus' && filterType === 'critical') {
                if(statusSelect) { statusSelect.value = 'critical'; controlViews.filterPcStatus(); }
            } else if (viewName === 'flagHistory' && filterType === 'recovery') {
                if(searchInput) { searchInput.value = '자동 복구'; controlViews.filterLogs(); }
            }
        }, 100);
    },
